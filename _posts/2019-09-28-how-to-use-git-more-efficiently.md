...
我之前有篇[文章][git-easy-tutorial]讲了 git 的各种命令，是为了在需要的时候方便查看。本篇我想从实际的工作场景出发，来看看如何通过更合理地使用 git，来提高我们的工作效率以及代码质量。

<h4>About Git</h4>

- [VCS][vcs] --- cvs/svn/mercurial
- Linus Torvalds
- Github/Gitlab
...
不同于其他 VCS，git 并不是将文件的`差异`作为数据来存储， 而是将每一次 commit 看做一个`快照`，当获取一个文件在不同版本之间的差异时，是将两个版本之间的快照进行比对。我们可以将 git 看作一个支持快照的迷你文件系统。
...
Git 中几乎所有操作都是在本地完成，然后再与远端仓库同步，这使得 Git 可以在没有网络的情况下使用。
...
<h4>不同场景如何合理地使用 Git</h4>

下面从几个实际的工作场景出发，来看看我们应该如何合理地使用 git。
...
在开发一个比较棘手的特性时，改一点提一点验一下，因为每次改动可能并没有实际的意义，因此会使用之前的 commit message 作为本次的 commit message，这就会造成分支上有很多相同类型的 commit，这时候 `git commit --amend` 就解决不了这个问题了，`git reset` 可以很方便地解决这个问题。
...
*注：上面的操作会使得本地分支跟远端仓库的同名分支呈非线性结构，需要用 git push origin your_feautre_branch **-f** 来强推*
...
*注：对于一个大特性的开发，可能经常需要 rebase master，为了减少解决冲突的工作，[git-rerere](https://git-scm.com/docs/git-rerere) 也许能帮到你*

<em style="color: orangered">实验7 - 用 git rebase 解决场景4的问题</em>
...
<em style="color: orangered">实验8 - 演示 git blame 和 git show</em>
...
**场景14. 每次都要经过中心仓库同步本地跟编译机的代码，很烦**

可以使用 git remote add 直接将编译机的仓库设置为本地仓库的远程仓库。

<em style="color: orangered">实验9 - 演示 git remote add</em>
...
<iframe src="//player.bilibili.com/player.html?aid=75145732&cid=128550209&page=1" width="100%" height="600px" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"> </iframe>
